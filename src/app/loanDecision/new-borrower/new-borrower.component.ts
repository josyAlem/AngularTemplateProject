...
import {  newBorrowerRequestModel } from '../model/requestModel';
...
export class NewBorrowerComponent implements OnInit {
  ...
  initVariables() {
    ...
    this.dataModel = new newBorrowerRequestModel().getDataModel();
  }

  submitForm(formValue: JSON) {
    this.reset();

    let model: any = plainToClass(newBorrowerRequestModel, formValue);
    model = _.pick(model, (value, key, obj) => {
      return _.isNull(value) == false;
    });
    this._dataSvc.getRoaProfitForNb(model).subscribe(
      (res) => {
